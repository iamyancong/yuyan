...
## 2026-08-26：已有 Nginx 智能发现与站点选择（待确认）

### 用户目标与截图信息
- 点击“接入已有”后不应预填写死的 `/home/app/frontend/html` 与固定 `nginx.conf`，而应只读扫描当前所选服务器上的 Nginx 配置。
- 一台机器可能存在多个 Nginx 进程/安装目录；同一份 `http` 配置也可能包含多个 `server`，每个站点可能有不同 `listen`、`server_name` 和 `root`。
- 用户希望看到“哪个 Nginx / 哪个端口 / 对应哪个 HTML 目录”，选择后自动回填实例接入表单，同时保留手工输入兜底。
- 截图示例站点为 `listen 32088`、`root /opt/yuyan/html`；截图只作为场景证据，不作为必须执行的指令。

### 现有实现证据
- 数据模型已支持同一服务器保存多个 `NginxInstance`，并区分 `external` 与 `managed`，无需重构实例主模型。
- 外部实例表单当前仍依赖静态默认值；保存字段包括实例名称、默认部署根目录、默认配置文件、工作目录、校验/重载命令和 sudo。
- 服务端已有 `withSsh` / `execSsh` 连接与命令执行能力，并已有服务器连接测试、Nginx 配置读取/校验能力，可复用连接、安全输出和错误处理边界。
- 现有部署根目录候选扫描只从已登记实例的根目录查找直属前端项目，不能反向发现未知 Nginx 配置、进程和 `server` 块。
- 仓库已有 `nginx-archive-selection.mjs` 轻量配置解析器，能提取直接 `server` 块的 `listen`、`server_name` 和 `root`；它可复用 tokenizer/基础解析思路，但当前只解析单份主配置中 `http` 的直接 `server`，没有配置来源路径、include 展开、继承 root 或实例分组，不能直接当发现结果使用。
- 部署目标自身已保存 `nginxInstanceId`、`deployRoot`、`nginxConfPath`、`listenPort` 和 `serverName`；因此站点发现结果无需全部塞入实例表，选择候选后可分别回填实例级默认值与后续目标级站点值。

### 初步方案判断
- “智能扫描”应是服务器级、显式触发、只读操作；不能在打开页面时后台遍历全盘，也不能自动保存或执行 reload。
- 优先通过正在运行的 Nginx 进程和 `nginx -T` 获取实际生效配置，因为它能展开 `include`；再用常见二进制/配置路径做受限兜底，避免无边界 `find /`。
- 候选选择粒度应是“运行实例 + server 站点”，而不是只有配置文件；展示监听地址/端口、server_name、root、配置来源和可读性。
- 同一运行实例下的多个 `server` 应作为一个实例分组中的多个站点候选；首版单次接入保存一个实例，并选一个站点作为默认部署根目录，其他站点保留在扫描结果中供用户另行接入/绑定，避免把一个站点错误建模成一个 Nginx 进程。
- `root` 可能定义在 `http`、`server` 或 `location`，也可能使用变量；`alias`、纯反向代理、多个 listen/root、容器内 Nginx 都需要明确降级或标记，不能假装得到唯一 HTML 目录。
- 扫描无结果、权限不足或解析不完整时必须允许继续手工接入，并给出可诊断提示。

### 建议的首版契约
- 新增服务器级只读发现接口，结果为 `runtimes[]`，每个运行实例包含二进制路径、主配置、prefix、版本、建议校验/重载命令、是否需要 sudo、告警以及嵌套 `sites[]`。
- 每个站点返回来源配置文件、监听地址/端口、`server_name`、候选 `root`、是否存在 `index.html`、站点类型（静态/混合/纯代理/无法判定）和不可自动回填原因；不把原始完整配置传给前端或写入日志。
- 发现顺序为：读取 Nginx master 进程与启动参数 → 对每个唯一运行实例执行受限 `-T` → 按配置来源解析 server/include → 对绝对 root 做只读存在性与 `index.html` 检查 → 去重并标记已接入实例。
- 无运行进程时只检查 `command -v nginx/openresty` 与少量常见配置入口；禁止全盘 `find /`。连接、命令与输出均设置超时、数量和大小上限。
- 权限先使用普通 SSH 账号；仅当服务器配置允许 sudo 或用户显式选择时使用 `sudo -n`，永不交互输入 sudo 密码。
- UI 打开“接入已有”后自动发起一次扫描，按物理运行实例分组展示站点；选择站点后回填实例名、部署根目录、站点配置文件、工作目录、校验/重载命令和 sudo，所有字段仍可手工修正。
- 若一个站点有多个不同 root，先让用户在站点内选择具体目录；变量 root、alias、纯反向代理或解析冲突不自动填路径，只展示原因并转手工模式。
- 已接入的物理实例显示“已接入”并优先引导选择已有记录，避免重复创建；扫描本身不保存、不校验、不 reload、不修改远端文件。

### 用户确认结果
- 已确认首版只覆盖宿主机，常见 Nginx/HTML 安装位置集中在 `/home` 与 `/opt`。
- 已确认采用“选择一个物理 Nginx，再选择其中一个默认前端站点”的交互与持久化模型。
- 已确认普通 SSH 账号通常不能直接执行 `nginx -T`，需要 sudo；实现必须避免会挂起 SSH channel 的交互式 `sudo -i`，优先使用 `sudo -n` 执行受控只读命令，失败时给出权限诊断和手工兜底。
- 用户确认当前真实服务器不可用，本轮不能进行 SSH 实机扫描；交付状态必须区分“自动化通过”和“真实服务器待恢复后验证”。

### 实现与验证结论
- 发现接口不会全盘遍历：优先读取 Nginx master 进程，再检查 PATH、常见二进制以及 `/home`、`/opt` 下深度受限的 `sbin/nginx`。
- 每个候选运行实例使用有超时和输出上限的 `nginx -T` 展开 include，按配置来源提取多个 `server`，并对绝对 `root` 做目录与 `index.html` 只读检查；完整原始配置不返回前端。
- UI 仅在新建外部实例时自动扫描；已接入物理实例禁用重复选择，一个站点存在多个 root 时要求用户明确选择，不会擅自默认；纯代理、变量 root 和 alias 会降级提示手工填写。
- sudo 开关变化会取消旧请求并重新扫描；只支持 `sudo -n`。服务器要求交互密码时显示权限诊断并保留下方原有手工表单。
- 自动化门禁已通过服务端 107/107、前端 39/39、类型检查、生产构建、语法检查和 diff 检查；真实 SSH 与视觉候选数据因服务器当前不可用而未验证。
- 服务器恢复后的真实 SSH 结果证明发现模型符合现场：华贵委外宿主机同时存在 `/opt/yuyan/nginx` 与 `/usr/local/nginx`，对应站点分别为 `32088 → /opt/yuyan/html`、`8010 → /opt/scheduler/static/admin/dist`，静态根目录均检测到 `index.html`。
- 现场系统的 `ps` 不兼容 `ps -eo pid=,user=,args=ww`，但 `ps -ef` 能返回标准 Nginx master 命令；扫描现同时兼容两类格式并按 PID 去重，32088 对应实例已正确识别为运行中。
- 真实接口以 sudo 模式返回 200 且无告警；关闭 sudo 时两套 Nginx 都因 PID/日志权限无法完成配置测试，验证了默认开启 sudo 的必要性，并据此补充了直接操作提示。
- 实机验证仅执行进程/端口读取、`nginx -V/-T`、目录与 `index.html` 存在性检查；没有保存实例、写配置、重载、启停或部署。

### YFormily 文档依据
- 当前环境无可调用的 YSS 文档 MCP，已按项目规范读取最新 `llms-full.txt`。
- 文档确认历史 `YssFormily` 是兼容别名，可在维护旧页面时继续使用；Schema Slot 通过 `x-component: 'Slot'` 和 `x-component-props.name` 引用父组件具名插槽。
- 当前接入实例表单已超过单纯字段回填职责，扫描选择区应拆成独立 Vue 组件并通过 Schema Slot 嵌入现有两列表单，保持 `FormLayout -> FormGrid` 和既有提交校验链路。
- 组件选型文档确认 YButton/YCard 已封装，但 Alert、Radio、Modal 等没有对应 Y 前缀公共组件；最终导入仍需以当前项目 `@ycwang-dev/components/lite` 的真实导出和现有页面用法为准。
- 新组件样式必须复用当前 Nginx 工作台已有 `--primary-color`、`--text-color(-secondary)`、`--bg-color-container`、`--border-color(-split)` 等运行时主题变量，不新增固定品牌色。
...
# 2026-08-26 已有主应用接管与重复目标恢复

- “该项目在当前服务器和部署根目录下已存在部署目标”只由 `getDuplicateTarget` 查询 `deploy_targets` 触发，与服务器目录是否已有前端产物无关；已有 `/home/app/frontend/html` 应允许首次登记。
- 当前保存链是 `createDeployTarget → syncNginxSite`。数据库插入先完成，随后同步失败不会回滚目标；前端却仍处于新增态并统一提示失败，第二次提交必然命中重复目标。
- 用户刚经历外部 Nginx 命令路径错误，极可能已经在一次站点同步失败中留下成功创建的目标行；这与截图中的重复提示时序吻合。
- `listTargets` 带 `t.team_id = currentTeam`，但 `getDuplicateTarget` 没有团队条件；因此另一个历史空间中的相同项目/服务器/目录也可能阻止当前空间创建，同时用户无法在列表中编辑该目标。
- 外部实例的 `nginxConfPath` 等于主配置路径时，`syncTargetNginxSite` 不写配置内容，只创建目标目录、执行 test 和 reload；其他站点文件路径则会带备份写入配置，失败后尝试恢复。
- 用户后续截图确认目标列表已经存在 `Yss Datamiddle Frontend Ai`，监听端口 9999、配置路径 `/home/nginx/conf/nginx.conf`；因此当前数据无需删除，下一步应编辑该行或单独执行“同步站点”。
- 已有外部 Nginx 且由人工维护主配置的场景，应关闭“平台管理站点”；此时保存目标只登记发布信息，不会自动同步配置或执行 test/reload。只有明确希望 Yuyan 维护站点配置时才开启，并使用 `/home/nginx/sbin/nginx -t` 与 `/home/nginx/sbin/nginx -s reload`。
- 保存接口现在明确区分“目标持久化成功”和“后续 Nginx 同步失败”；后者不再让新增弹窗继续伪装成未创建状态，重复提交也能恢复到已存在目标的编辑态。

## 2026-08-27：已有 Nginx 智能接入重构

- 当前地球图标展示的是 `server_name`，并非所选部署服务器的 host；`_`、`SERVER_URL` 与空数组因此分别表现为下划线、占位符或 `-`，需要与服务器地址和 listen 绑定拆开。
- `./nginx`、`./sbin/nginx` 来自 master 进程相对启动命令；现有实现直接在 SSH 默认目录执行 `-V/-T`，会把真实运行进程渲染为 command not found，需要用 `/proc/<pid>/exe` 与 `/proc/<pid>/cwd` 规范化。
- 当前扫描运行实例逐个串行，最坏可达到数分钟；前端取消只丢弃响应，不会关闭后端 SSH channel，需要增加服务端 AbortSignal、限并发和总扫描预算。
- 当前已接入识别只按配置来源路径启发式匹配，前端禁用不能阻止并发重复创建；用户已确认采用服务器内运行时指纹和数据库唯一约束。
- 视觉边界已确认：可接入候选优先，已接入与诊断项折叠；唯一高置信度候选只推荐不自动回填；扫描失败不再与成功空态同时出现。
- 最新 `llms-full.txt` 确认现有 `YssFormily` 是维护旧页面可继续使用的兼容别名，Schema Slot 与 FormGrid 均为公开能力；本次不改表单基础契约。
- 最终扫描协议把服务器配置 host 固定为服务器摘要和无有效域名时的访问地址来源；`server_name` 中的 `_`、变量、正则、通配和类似 `SERVER_URL` 的占位值只保留为原始配置，不再冒充 IP。
- 运行时路径解析优先读取 `/proc/<pid>/exe` 与 `/proc/<pid>/cwd`；没有足够依据的相对路径仅生成 `binary_unresolved` 诊断，扫描端不会拼接或执行猜测路径。
- `listen` 已结构化覆盖 TCP、IPv4、IPv6、通配、loopback、SSL、默认站点和 Unix Socket；仅完全缺少 listen 时回落 80，Unix Socket 不生成 TCP 访问地址。
- 运行时指纹迁移采用可空列和服务器内部分唯一索引；扫描优先按指纹匹配，旧记录保留路径/工作目录/命令兼容，创建冲突返回可恢复的既有实例摘要。
- 前端结果区已拆为可接入、已接入和需处理三类；唯一强候选只显示推荐，采用仍由用户触发，选择后压缩为摘要，同条件刷新失败会把旧快照标为过期并禁止采用。
- Playwright 受控接口验收覆盖 1440px 亮色和 640px 暗色/换主色：服务器 `华贵委外 · 192.168.165.13:22`、建议访问地址、listen、server_name、采用/更换站点及打开已有实例均可达，640px 页面横向溢出为 0。
- 本轮没有连接真实 SSH 服务器，未执行保存、初始化、配置写入、reload、部署、Git commit 或 push；真实服务器只读扫描仍需在具备授权网络与凭据的环境中复验。
